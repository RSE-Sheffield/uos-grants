...
class State(TypedDict):
    ...
    intent: str
    summary_context: Optional[dict]  # <-- new field for important facts
...
# === Summary Extractor ===
def extract_summary_context_factory(llm):
    def extract_summary_context(state: State) -> State:
        # Pull relevant new AI content
        ...
        if latest_response is None:
            return state  # or optionally just return unchanged state

        prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessage(
                    """
                From the following response, extract key named entities or references (e.g. people, grants, topics) 
                and their relevance. Return a concise dictionary-style summary of the information that may be needed later.
                If nothing useful is present, return an empty dict.
                """
                ),
                HumanMessage(content=latest_response.content),
            ]
        )

        summary_str = llm.invoke(prompt.format_messages()).content.strip()
        try:
            new_summary = (
                eval(summary_str, {}, {})
        ...
        # Merge into existing context (overwrite keys or append to lists)
        combined_summary = state.get("summary_context", {}).copy()
        for k, v in new_summary.items():
            if isinstance(v, list):
                combined_summary.setdefault(k, []).extend(v)
            else:
                combined_summary[k] = v

        return {**state, "summary_context": combined_summary}

    return extract_summary_context
...
def generate_response_factory(llm):
    async def generate_response(state: State):
        ...
        if state.get("summary_context"):
            for k, items in state["summary_context"].items():
                if isinstance(items, list):
                    ...
                    structured += f"{k.capitalize()}: {items}\n"
        ...
        system_content = (
            ...
            "Based on the context provided, answer user queries about people, grants, or departments.\n"
            "If asked why someone was recommended, refer to previously shared relevant context.\n"
            "Do not invent information.\n\n"
            f"Previously extracted summary:\n{structured}\n"
            f"Raw content:\n{retrieved_context}"
        )
...
def make_graph(llm, retriever, memory=None):
    ...
    graph.add_node(
        "extract_summary_context", extract_summary_context_factory(llm)
    )
    ...
    # Add summarisation path
    for node in [
        "retrieve_context",
        "reuse_previous_context",
        "lookup_by_name",
        "compare",
        "chitchat",
    ]:
        graph.add_edge(node, "extract_summary_context")

    graph.add_edge("extract_summary_context", "generate_response")
