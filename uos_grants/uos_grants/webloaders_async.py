...
class AcademicDeptScraperAsync_Old:
    def __init__(
        self,
        url: str,
        db_session_factory,
...
class AcademicDeptScraperAsync:
    def __init__(
        self,
        url: str,
        db_session_factory,
        ...
        try:
            async with session.get(url) as response:
                ...
                if "<html" not in text.lower():
                    logger.warning(
                        f"[WARN] 200 OK but no HTML content at {url}"
                    )
                return text
                # return str(response.status)
        except Exception as e:
        ...
        async def run():
            ...
            try:
                ...
                researcher = await asyncio.to_thread(
                    Researcher, self.temp_dict[url], url
                )
                data = researcher.to_dict()
                data["last_response"] = "200"
            except Exception as e:
                logger.warning(
                    f"[ERROR] Failed to parse researcher at {url}: {e}"
                )
                self._write_to_file({url}, "missed_profiles.txt")
                return
